"""Sell Crude Oil HIGH $100 NO position — oil at $99.24, too close to trigger."""
...
if receipt.get('success') or receipt.get('orderID'):
    ...
    tg(f"<b>🔴 SELL: Crude Oil $100 NO</b>\n{sell_shares:.1f} shares @ {sell_price:.3f}\nProceeds: ~${proceeds:.2f}\nReason: WTI at $99.24 — too close to $100 trigger\nOriginal cost: $182 | Loss locked: ~${proceeds-182:.0f}\nOrder: {oid[:20]}...")
else:
    err = receipt.get('errorMsg', str(receipt))
    print(f"❌ {err}")
    tg(f"<b>SELL FAILED: Crude Oil NO</b>\n{err[:100]}")
