...
def patch_executor():
    ...
    FLAG = "/opt/polymarket-agent/.executor_patched_v6"
    ...
    with open("/opt/polymarket-agent/executor_dump.txt", "w") as _df:
        _df.write(code)
    log("  [PATCH] executor dumped to /opt/polymarket-agent/executor_dump.txt")
    print("EXECUTOR_DUMP:" + code[-3000:])
