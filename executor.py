...
def verify_signature(body_bytes: bytes, sig_header: str) -> bool:
    ...
    Note: Replay protection removed — HMAC-SHA256 with a 64-char secret is
    sufficient for our threat model (private server, no public exposure).
    Replay protection caused false 401s on hourly cron agents sending identical
    payloads (same command + script name = same body = same signature within window).
    """
    if not SECRET:
