...
# BLG -1.5 YES token
YES_TOKEN = "21599083154255901481562620383387883369629723532742307924043518715415168217676"
SIZE = 50
...
if best_ask > 0.99:
    print(f"Market may be resolved ({best_ask:.3f}) — aborting"); exit(1)
...
print(f"Buying {shares} BLG -1.5 @ {buy_price} = ${shares*buy_price:.2f}")
...
if (result.get("success") or result.get("orderID")) and TG_TOKEN and TG_CHAT:
    msg = f"<b>🎮 BLG -1.5 placed</b>\n{shares:.1f} shares @ {buy_price:.3f} = ${shares*buy_price:.2f}\ndkxbt call: BLG to win 2-0 vs WE"
